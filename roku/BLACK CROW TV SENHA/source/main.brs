...
sub Main()
    print "===== Iniciando Aplicativo Roku ====="
    
    screen = CreateObject("roSGScreen")
    m.port = CreateObject("roMessagePort")
    screen.setMessagePort(m.port)

    ' URL do arquivo de senha
    senhaUrl = "https://junior7766.github.io/Junior7766/roku/senha.txt"

    ' Criando objeto para buscar senha
    print "Buscando senha..."
    urlTransfer = CreateObject("roUrlTransfer")
    urlTransfer.SetCertificatesFile("common:/certs/ca-bundle.crt")
    urlTransfer.SetUrl(senhaUrl)
    
    fileContent = urlTransfer.GetToString()
    
    if fileContent = invalid then
        print "Erro: Falha ao obter resposta do servidor!"
        fileContent = "" ' Evita erros de comparação
    end if

    print "Conteúdo da senha (bruto): [" + fileContent + "]"
    
    ' Removendo espaços extras
    senhaLimpa = fileContent.Replace(" ", "").Replace(Chr(10), "").Replace(Chr(13), "")
    print "Senha obtida (limpa): [" + senhaLimpa + "]"

    ' Se a senha estiver incorreta, exibe a tela de "Aplicativo Desativado"
    if senhaLimpa <> "0099" then
        print "Senha incorreta! Exibindo tela de Aplicativo Desativado."
        disabledScene = screen.CreateScene("DisabledScene")
        screen.show()
    ...
    ' Se a senha estiver correta, segue fluxo normal
    print "Senha correta! Carregando HomeScene..."
    ...
    while(true)
       ...
       if m.global.AdTracker = 0 then
           if msg.GetField() = "rowItemSelected" or msg.GetField() = "fire" then
                RAF()
           end if
       end if
